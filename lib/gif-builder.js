...
function makeBgGif(pcol, W, H) {
  ...
  const circles = Array.from({ length: NC }, (_, i) => ({
    ...
    r:     55 + Math.round(srand(i*11+2) * 75),   // 55-130 px
    dx:    (srand(i*11+3)-0.5) * 28,              // ±14 px drift
    dy:    (srand(i*11+4)-0.5) * 18,
    start: i / NC,                                 // evenly spread entry points
    dur:   0.38 + srand(i*11+5) * 0.28,           // active 38-66% of cycle
  }));
  ...
  const frames = Array.from({ length: NF }, (_, fi) => {
    ...
    for (const c of circles) {
      ...
      const alpha = Math.sin(Math.PI * lifeT);                  // smooth bell
    ...
    const gs = 0.025 + 0.028 * (0.5 - 0.5 * Math.cos(2 * Math.PI * t));
    const GW = 190;
    for (let y = 0; y < H; y++) {
      for (let x = 0; x < GW; x++) {
