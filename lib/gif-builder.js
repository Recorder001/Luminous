...
//
function makeBgGif(pcol, W, H, seedOffset = 0) {
  // sr(): srand with per-request seed mixed in
  const sr = n => srand(n + seedOffset * 997);
  ...
  const stars = Array.from({ length: NSTARS }, (_, i) => ({
    x:     Math.round(sr(i*7+0) * W),
    y:     Math.round(sr(i*7+1) * H),
    cross: sr(i*7+2) < 0.35,
    phase: sr(i*7+3) * Math.PI * 2,
    speed: 0.4 + sr(i*7+4) * 2.0,
  }));

  // ── Circle layout (seeded, staggered phases) ─────────────────
  const NC = 5;
  const circles = Array.from({ length: NC }, (_, i) => ({
    cx:    Math.round(sr(i*11+0) * W),
    cy:    Math.round(sr(i*11+1) * H),
    r:     60 + Math.round(sr(i*11+2) * 80),
    dx:    (sr(i*11+3)-0.5) * 40,
    dy:    (sr(i*11+4)-0.5) * 24,
    start: i / NC,
    dur:   0.40 + sr(i*11+5) * 0.25,
  }));
  ...
  const frames = Array.from({ length: NF }, (_, fi) => {
    ...
    for (const c of circles) {
      ...
      const palIdx = Math.max(1, Math.min(7, Math.round(alpha * 6) + 1));

      // Radius grows continuously from r to 1.3×r over full lifetime
      const expansion = 1.0 + lifeT * 0.3;
